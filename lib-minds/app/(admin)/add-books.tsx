...
const AddBooks = () => {
  ...
  const IMGUR_CLIENT_ID = '09933ebcb99eeff';
  ...
  const handleAddBook = async () => {
    ...
    if (isNaN(maxBorrowDaysNum) || maxBorrowDaysNum <= 0) {
      alert('Max borrow days must be a positive number.');
      return;
    }

    setLoading(true);
    try {
      const imgUrl = await uploadToImgur(image);
      ...
      >
        ...
        <ScrollView className="flex-1 mx-2" showsVerticalScrollIndicator={false}>
          <Pressable onPress={() => router.back()} className="mb-4 mt-2 flex flex-row items-center">
            ...
            <Text className="px-2 text-xl font-semibold" style={{ color: headingColor }}>
              Back
            </Text>
          </Pressable>
